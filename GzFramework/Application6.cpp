...
int GzAntiAlias(GzDisplay **display, GzRender **render);
...
int Application6::Render()
{
	...
	* and render each triangle
	*/
	std::vector<GzTriangle*> triangleList;
	while (fscanf(infile, "%s", dummy) == 1) { 	/* read in tri word */
		fscanf(infile, "%f %f %f %f %f %f %f %f",
			&(vertexList[0][0]), &(vertexList[0][1]),
			&(vertexList[0][2]),
...
int GzAntiAlias(GzDisplay **display, GzRender **render) {
	GzNewDisplay(display, render[0]->display->dispClass, render[0]->display->xres, render[0]->display->yres);
	...
	for (int i = 0; i < (*display)->yres; i++) {
		for (int j = 0; j < (*display)->xres; j++) {
			...
			alpha = 0;
			zBuf = 0;
			for (int alias = 0; alias < AAKERNEL_SIZE; alias++) {
				GzGetDisplay(render[alias]->display, j, i, &r, &g, &b, &a, &z);

				red		+= ((float)r) * AAFilter[alias][2];
				green	+= ((float)g) * AAFilter[alias][2];
				blue	+= ((float)b) * AAFilter[alias][2];
				alpha	+= ((float)a) * AAFilter[alias][2];
				zBuf	+= ((float)z) * AAFilter[alias][2];
			}
			red <<= 4;	green <<= 4;	blue <<= 4;	alpha <<= 4;
			GzPutDisplay(*display, j, i, (GzIntensity)red, (GzIntensity)green, (GzIntensity)blue, (GzIntensity)alpha, (GzDepth)zBuf);
		}
	}
